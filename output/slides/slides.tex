...
\begin{frame}{Construction and Agriculture by State}
  ...
  \textbf{Agriculture:} North Dakota (29.6 p.p.) now leads, ahead of Michigan
  (23.5 p.p.) and Alaska (23.1 p.p.); Montana and Idaho (previously \#1/\#2)
  remain in the top 5.}
\end{frame}
    ...
    \begin{column}{0.50\textwidth}
      \begin{table}
        ...
        \begin{tabular}{lcc}
          ...
          Also national same tail & 11.4\% & 2.7\% \\
          \bottomrule
        \end{tabular}
      \end{table}
      \vspace{0.4em}
...
\begin{frame}{Flow vs.\ Stock: Motivation}
  \begin{itemize}
    ...
    \end{itemize}
    \bigskip
    \item \textbf{Result: correlated, more so than first measured.}
    \begin{itemize}
      \item National ($n=997$): Pearson $r = 0.784$, Spearman $r = 0.660$
      \item State $\times$ NAICS6 ($n=39{,}446$): Pearson $r = 0.750$,
            Spearman $r = 0.661$
      \item \textbf{Peak-quarter concordance is still low --- $\sim$15\%
    ...
    \item[$\to$] Timing, not just amplitude, differs between ``separations
          spike'' and ``headcount swings'' --- but the two are more tightly
          linked in overall magnitude than an earlier data-thinness bug made
          them look (see next slide)
  \end{itemize}
\end{frame}
...
\begin{frame}{Flow vs.\ Stock: Sector Heterogeneity}
  \begin{columns}
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{itemize}
        \item $r = 0.64$ ($n=55$), revised up sharply from an original
              $r \approx 0.21$ --- that estimate was itself a data-thinness
              artifact (a few single-quarter cells dragging it down); the
              ``rapid worker replacement'' story is retracted
      \end{itemize}
      \smallskip
      \textbf{``Stock without flow,'' updated}
      \begin{itemize}
        \item Cotton Ginning (flow 0.01, stock 0.82) pairs with Political
              Organizations (flow 0.18, stock 0.83) --- hiring surges (harvest;
              elections) without a matching separation spike. Tax Prep
              Services dropped out entirely (flow now correctly undefined)
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Summary}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Flow vs.\ stock:} separations-based and employment-based
          ...
          ($r=0.97$), Agriculture moderate ($r=0.64$, revised up from an
          original 0.21 that turned out to be a data-thinness artifact)
    \smallskip
