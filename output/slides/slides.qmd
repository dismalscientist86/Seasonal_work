...
: Top 12 most seasonal 6-digit NAICS industries (national)
...
- $n$ = years contributing to ExcessPeak estimate; a $\geq 5$-year minimum
  ...
  extreme seasonality
- Non-agriculture entries are intuitive once named: mobile food services
  (food trucks), drive-in theaters, recreational goods rental
- Seasonal index normalized to [0,1] via 99th-pct winsorization ---
  raw amplitude is right-skewed (median 0.03, max 0.96), so dividing
  by the true max would compress everyone else near 0; cost is that
  the top ~10 industries all tie at exactly 1.00
...
: Mean seasonal index by 2-digit sector (national)

Education peaks in Q2 (end of school year), not Q4 --- ExcessPeak is
positive across all sectors. CP's 1-digit rankings confirmed at finer
granularity.
...
- We compute ExcessPeak for each state $\times$ 6-digit NAICS cell (42,344
  cells with $\geq 5$ year-observations)

- **Main finding:** a 2.8$\times$ gap between most and least seasonal
...
1. Agriculture (cross-state SD = 14.5 p.p.)
2. Arts & entertainment (SD = 13.4 p.p.)
3. Accommodation/food (SD = 10.8 p.p.)
4. Construction (SD = 9.6 p.p.)

- **Least geographically variable:** healthcare, metals manufacturing,
